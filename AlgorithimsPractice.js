...
//  always be higher than the base.For example, if the parameters passed are (2, 6),
//  the function should return [2, 4, 6] as 2, 4, and 6 are the multiples of 2 up to 6.

function findMultiples(integer, limit) {
	// create an array to push elements in to
	const arr = [];
	// iterate starting at integer ending when integer is greater/equal to limit
	for (let i = integer; i <= limit; i++) {
		// if i is divisible by integer
		if (i % integer === 0) {
			// then push i into array
			arr.push(i);
		}
	}
